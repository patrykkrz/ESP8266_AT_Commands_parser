...
void ESP_Main_Thread(void const* params) {
    ...
    /* Configure GPIO pin as GPIO mode with pull up enabled and output direction */
    gpio.Mode = ESP_GPIO_Mode_GPIO;
    gpio.Pull = ESP_GPIO_Pull_UpEnabled;
    gpio.Dir = ESP_GPIO_Dir_Output;
    
    /* Configure pin 12 setup except direciton */
    if ((espRes = ESP_SYS_GPIO_SetConfig(&ESP, 12, &gpio, 1)) == espOK) {
        printf("GPIO config set ok!\r\n");
    } else {
        printf("GPIO config setup failed. Status: %d\r\n", espRes);
    }
    /* Configure pin 12 as output */
    if ((espRes = ESP_SYS_GPIO_SetDir(&ESP, 12, &gpio, 1)) == espOK) {
        printf("GPIO direction set ok!\r\n");
    } else {
        printf("GPIO direction setup failed. Status: %d\r\n", espRes);
    }
    /* Write pin 12 to HIGH */
    if ((espRes = ESP_SYS_GPIO_Write(&ESP, 12, 1, 1)) == espOK) {
    ...
    while (1) {
        ...
        if (TM_DISCO_ButtonOnPressed()) {   /* Handle button press */
            ...
            /* Read pin 12 value after write */
            if ((espRes = ESP_SYS_GPIO_Read(&ESP, 12, &level, NULL, 1)) == espOK) {
                printf("GPIO read level ok. Level: %d!\r\n", level);
            } else {
